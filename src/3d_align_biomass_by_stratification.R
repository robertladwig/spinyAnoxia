...
strat <- read.csv(file = "data_processed//stratification_start.csv") # all in year 2000 dates FYI

phyto <- readRDS("data_processed/3b_phyto_split_by_year.rds")

output.folder <- "data_processed/3d_biomass_aligned_by_strat_onset/"

file.phyto.interps.all <- "data_processed/3d_biomass_aligned_by_strat_onset_interp_tables_all.rds"

file.phyto.SWF.comparison <- "data_processed/3d_biomass_aligned_by_strat_onset_interp_tables_pre-post.rds"
...
strat$constant.high <- yday(parse_date_time(x = strat$constant.high, orders = "ymd"))
